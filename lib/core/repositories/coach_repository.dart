...
class CoachRepository {
  ...
  Future<List<Coach>> getCoachesBySpecialty(String specialty) async {
    try {
      ...
      AppPrint.printSuccess(
        'Successfully fetched ${filteredCoaches.length} coaches with specialty: $specialty',
      );
  ...
  Future<List<Coach>> getCoachesByCertification(String certification) async {
    try {
      ...
      AppPrint.printSuccess(
        'Successfully fetched ${filteredCoaches.length} coaches with certification: $certification',
      );
  ...
  Future<Coach?> updateCoachCertifications(
    String coachId,
    List<String> certifications,
  ) async {
    try {
      ...
      final updateData = {'certifications': certifications};
  ...
  Future<Coach?> updateCoachSpecialties(
    String coachId,
    List<String> specialties,
  ) async {
    try {
      ...
      final updateData = {'specialties': specialties};
  ...
  Future<Coach?> addCoachCertification(
    String coachId,
    ...
    try {
      ...
      final updatedCertifications = List<String>.from(
        currentCoach.certifications,
      );
  ...
  Future<Coach?> removeCoachCertification(
    String coachId,
    String certification,
  ) async {
    try {
  ...
  Future<List<Coach>> getCoachesWithPagination({
    ...
    try {
      AppPrint.printStep(
        'Fetching coaches with pagination (limit: $limit, offset: $offset)',
      );
      final startTime = DateTime.now();
  ...
  Future<List<Coach>> searchCoaches(String searchTerm) async {
    try {
      ...
      final filteredCoaches = coaches.where((coach) {
        ...
        return (coach.bio?.toLowerCase().contains(searchLower) ?? false) ||
            coach.specialties.any(
              (specialty) => specialty.toLowerCase().contains(searchLower),
            ) ||
            coach.certifications.any(
              (cert) => cert.toLowerCase().contains(searchLower),
            ) ||
      ...
      AppPrint.printSuccess(
        'Found ${filteredCoaches.length} coaches matching: $searchTerm',
      );
